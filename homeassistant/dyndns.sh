...
if [ -z "$VPN_ENDPOINT_DOMAIN" ]; then
  echo "$(date +"%m/%d/%Y %H:%M:%S"): VPN_ENDPOINT_DOMAIN is not set in the .env file."
  exit 1
fi
...
if [ -z "$NEW_IP" ]; then
  echo "$(date +"%m/%d/%Y %H:%M:%S"): Failed to resolve IP for domain: $VPN_ENDPOINT_DOMAIN"
  exit 1
fi
...
if [ "$CURRENT_IP" != "$NEW_IP" ]; then
  echo "$(date +"%m/%d/%Y %H:%M:%S"): Updating wg0.conf Endpoint from $CURRENT_IP to $NEW_IP"
